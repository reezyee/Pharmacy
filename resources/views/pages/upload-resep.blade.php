...
@section('content')
    <div class="max-w-2xl mx-auto bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-lg border border-white/20">
        <h2 class="text-2xl font-medium mb-6 text-gray-800">Upload Recipes</h2>
        ...
        <form id="upload-form" action="{{ route('resep.upload') }}" method="POST" enctype="multipart/form-data"
            ...
            <!-- Pilihan Pasien -->
            <div class="mb-8">
                <label class="block text-sm font-medium text-gray-700 mb-3">Patient Name</label>
                <div class="flex items-center space-x-6 bg-gray-50/80 backdrop-blur-sm p-4 rounded-lg shadow-sm">
                    <label class="flex items-center space-x-3">
                        <div class="relative">
                            <input type="radio" name="patient_type" value="self" checked
                                class="form-radio w-5 h-5 text-blue-500 focus:ring-blue-400">
                            <span
                                class="ripple absolute inset-0 w-10 h-10 -top-2.5 -left-2.5 rounded-full pointer-events-none"></span>
                        </div>
                        <span class="text-gray-800">Self ({{ auth()->user()->name }})</span>
                    </label>
                    <label class="flex items-center space-x-3">
                        <div class="relative">
                        ...
                        <span class="text-gray-800">Other Patient</span>
                    </label>
                </div>
                <div class="mt-4 relative">
                    <input type="text" id="patient_name" name="pasien_nama" placeholder="Other Patient Name"
                        class="hidden py-3 px-4 border-b border-gray-300 focus:border-blue-500 transition-all duration-300 bg-transparent w-full focus:outline-none"
                        disabled>
            ...
            <!-- Unggah Foto -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-3">Upload Photo/Scan Recipes</label>
                <div
                    class="relative bg-gray-50/80 backdrop-blur-sm p-6 rounded-lg border border-gray-200 shadow-sm transition-all hover:shadow-md">
                    ...
                    <div class="text-center">
                        ...
                        <p class="mt-2 text-gray-500">Click or drag file here</p>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-center mb-4">
                <hr class="w-[40%]">
            <p class="text-center text-gray-300 w-[20%]">
                Or
            </p>
            <hr class="w-[40%]">
            </div>
            ...
            <button type="button" onclick="showCamera()"
                class="w-full flex items-center justify-center bg-blue-500 text-white px-6 py-3 rounded-lg text-base font-medium shadow-md hover:bg-blue-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50 transform hover:-translate-y-0.5">
            ...
            <div id="camera-section"
                class="hidden mt-8 bg-white/90 backdrop-blur-sm p-5 rounded-lg border border-gray-200 shadow-lg">
                <video id="camera" class="w-full rounded-lg border overflow-hidden" playsinline></video>

                <div class="flex space-x-3 mt-4">
                    <button type="button" onclick="switchCamera()"
                        class="flex-1 flex items-center justify-center bg-gray-500 text-white px-4 py-2 rounded-lg font-medium shadow-md hover:bg-gray-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-opacity-50">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <button title="Take Photo" type="button" onclick="capturePhoto()"
                        ...
                        {{-- <svg class="w-14 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="bg-white border border-gray-700 w-12 h-12 rounded-full hover:bg-slate-100"></div>
                    </button>
                    <button type="button" onclick="closeCamera()"
                        class="flex-1 flex items-center justify-center bg-red-500 text-white px-4 py-2 rounded-lg font-medium shadow-md hover:bg-red-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-red-300 focus:ring-opacity-50">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- Preview Foto -->
            <div id="photo-preview" class="mt-6 flex justify-start flex-wrap gap-4"></div>

            <!-- Tombol Submit -->
            <button type="submit"
                class="w-full mt-8 flex items-center justify-center bg-green-500 text-white px-6 py-3 rounded-lg text-base font-medium shadow-md hover:bg-green-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-300 focus:ring-opacity-50 transform hover:-translate-y-0.5">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg">
    ...
    <script>
        ...
        // Fungsi untuk mengirim data saat submit form
